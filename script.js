...
function appendNewCard(parentElement) {
  ...
  //cardDown = cardElement.innerHTML;
  
  let cardUp = document.createElement("div");
  cardUp.setAttribute("class", "card-up");
  //cardUp = cardElement.innerHTML;
...
function shuffleCardImageClasses() {
  ...
  let imagesArray = ["image-1", "image-1", "image-2", "image-2", "image-3", "image-3", "image-4", "image-4", "image-5", "image-5", "image-6", "image-6"];
   ...
   */
  let shuffledImagesArray = _.shuffle(imagesArray);
   
  // Step 3: Return the shuffled array of class names.
  return shuffledImagesArray;
...
function createCards(parentElement, shuffledImageClasses) {
  ...
  for (let j = 0; j < 12; j++) {
    ...
    newCard.className += " ";
    newCard.className += shuffledImageClasses[j];
    let cardObject = {index: j, element: newCard, imageClass: shuffledImageClasses[j]};
...
function onCardFlipped(newlyFlippedCard) {
  ...
  if (doCardsMatch(newlyFlippedCard, lastCardFlipped)) {
    ...
    newlyFlippedCard.element.classList.add("match-animation");
    lastCardFlipped.element.classList.add("match-animation");
  }
